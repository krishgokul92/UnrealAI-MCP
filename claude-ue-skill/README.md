...
A comprehensive Claude Code skill for Unreal Engine development with deep project structure understanding, best practices, and **Blueprint clipboard text generation** (T3D serialization).
...
- **Blueprint Text Generation**: Generates valid, copy-pasteable Blueprint node graphs in UE's T3D clipboard format
...
5. **Generates Blueprint clipboard text**:
   - Produces valid T3D serialized node graphs
   - Copy-paste directly into UE5 Blueprint Editor (Ctrl+V)
   - Handles K2Node types, pin wiring, GUIDs, and bidirectional connections
   - Supports function calls, macros, branches, events, variables, and casts
...
- Blueprint clipboard text generation (T3D serialization)
- Blueprint node graph wiring and GUID management
...
- **Blueprint Serialization** (`references/blueprint_serialization.md`): Complete T3D format spec, K2Node types, pin categories, function references, and pasteable examples
...
- "Generate a Blueprint that finds all static meshes with override materials and prints their names"
- "Create a Blueprint node graph for a health pickup system"
