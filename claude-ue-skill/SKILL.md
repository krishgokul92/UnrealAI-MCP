...
description: >
  ...
  Blueprint clipboard serialization (generating pasteable T3D text for node graphs), Blueprint text generation,
...
User asks for Unreal help
    ...
    │   ├─> Blueprint generation? ─> Load references/blueprint_serialization.md
    │   │   ├─> Generate T3D clipboard text following serialization spec
    │   │   ├─> Verify all LinkedTo references are bidirectional
    │   │   ├─> Ensure unique GUIDs for all nodes and pins
    │   │   └─> Output as .txt file for user to Ctrl+V paste into UE editor
...
Before generating Blueprint clipboard text:
...
- [ ] Input pins do NOT have a `Direction` field
...
### references/blueprint_serialization.md
Load when generating Blueprint clipboard text (T3D format):
- Complete T3D serialization format specification
- K2Node types (CallFunction, MacroInstance, IfThenElse, Event, VariableGet/Set, DynamicCast, etc.)
- Pin specification (categories, container types, direction, visibility, LinkedTo syntax)
- Common function references (30+ Blueprint nodes mapped to MemberParent/MemberName)
- Common class paths for PinSubCategoryObject references
- Standard Macro GUIDs (ForEachLoop, ForLoop, IsValid)
- Struct default value formats (LinearColor, Vector, Rotator, Transform)
- Complete node examples (PrintString, ForEachLoop, Branch)
- Node layout best practices (spacing, flow direction)
- UE Editor paste workflow (Ctrl+V into Event Graph)
- Known limitations and version sensitivity notes
