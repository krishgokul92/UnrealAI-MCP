...
class FUnrealAIBlueprintGraphCommands
{
public:
    FUnrealAIBlueprintGraphCommands();
    ~FUnrealAIBlueprintGraphCommands();
...
private:
    ...
    // Paste T3D-format node text into a Blueprint graph (Path A workflow)
    TSharedPtr<FJsonObject> HandlePasteNodesToBlueprint(const TSharedPtr<FJsonObject>& Params);
};
