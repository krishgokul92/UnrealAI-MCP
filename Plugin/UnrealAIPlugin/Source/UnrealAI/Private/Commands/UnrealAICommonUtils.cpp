...
// JSON Utilities
TSharedPtr<FJsonObject> FUnrealAICommonUtils::CreateErrorResponse(const FString& Message)
{
...
TSharedPtr<FJsonObject> FUnrealAICommonUtils::CreateSuccessResponse(const TSharedPtr<FJsonObject>& Data)
{
    TSharedPtr<FJsonObject> ResponseObject = MakeShared<FJsonObject>();
...
void FUnrealAICommonUtils::GetIntArrayFromJson(const TSharedPtr<FJsonObject>& JsonObject, const FString& FieldName, TArray<int32>& OutArray)
{
    OutArray.Reset();
...
void FUnrealAICommonUtils::GetFloatArrayFromJson(const TSharedPtr<FJsonObject>& JsonObject, const FString& FieldName, TArray<float>& OutArray)
{
    OutArray.Reset();
...
FVector2D FUnrealAICommonUtils::GetVector2DFromJson(const TSharedPtr<FJsonObject>& JsonObject, const FString& FieldName)
{
...
FVector FUnrealAICommonUtils::GetVectorFromJson(const TSharedPtr<FJsonObject>& JsonObject, const FString& FieldName)
{
...
FRotator FUnrealAICommonUtils::GetRotatorFromJson(const TSharedPtr<FJsonObject>& JsonObject, const FString& FieldName)
{
...
// Blueprint Utilities
UBlueprint* FUnrealAICommonUtils::FindBlueprint(const FString& BlueprintName)
{
    return FindBlueprintByName(BlueprintName);
}

UBlueprint* FUnrealAICommonUtils::FindBlueprintByName(const FString& BlueprintName)
{
    // The correct object path for a Blueprint asset is /Game/Path/AssetName.AssetName
    FString ObjectPath;
    const FString TrimmedBlueprintName = BlueprintName.TrimStartAndEnd();
    const FString RequestedAssetName = FPaths::GetBaseFilename(TrimmedBlueprintName);

    // Check if BlueprintName is already a full path (starts with /)
    if (TrimmedBlueprintName.StartsWith(TEXT("/")))
    {
        // It's already a package or object path under /Game.
        if (TrimmedBlueprintName.Contains(TEXT(".")))
        {
            ObjectPath = TrimmedBlueprintName;
        }
        else
        {
            ObjectPath = FString::Printf(TEXT("%s.%s"), *TrimmedBlueprintName, *RequestedAssetName);
        }
    }
    else
    {
        // It's just a name, add the default /Game/Blueprints/ prefix
        ObjectPath = FString::Printf(TEXT("/Game/Blueprints/%s.%s"), *TrimmedBlueprintName, *TrimmedBlueprintName);
    }
    ...
    FString PackagePath = TEXT("/Game/Blueprints/") + TrimmedBlueprintName;
    Blueprint = FindObject<UBlueprint>(nullptr, *PackagePath);

    if (!Blueprint && !RequestedAssetName.IsEmpty())
    {
        FARFilter Filter;
        ...
        {
            if (!CandidateAsset.IsValid() || !CandidateAsset.AssetName.ToString().Equals(RequestedAssetName, ESearchCase::CaseSensitive))
            {
                continue;
            }

            Blueprint = Cast<UBlueprint>(CandidateAsset.GetAsset());
            if (Blueprint)
            {
                return Blueprint;
            }
...
UEdGraph* FUnrealAICommonUtils::FindOrCreateEventGraph(UBlueprint* Blueprint)
{
    if (!Blueprint)
    {
        return nullptr;
...
UK2Node_Event* FUnrealAICommonUtils::CreateEventNode(UEdGraph* Graph, const FString& EventName, const FVector2D& Position)
{
    if (!Graph)
    {
        return nullptr;
...
UK2Node_CallFunction* FUnrealAICommonUtils::CreateFunctionCallNode(UEdGraph* Graph, UFunction* Function, const FVector2D& Position)
{
    if (!Graph || !Function)
    {
        return nullptr;
...
UK2Node_VariableGet* FUnrealAICommonUtils::CreateVariableGetNode(UEdGraph* Graph, UBlueprint* Blueprint, const FString& VariableName, const FVector2D& Position)
{
    if (!Graph || !Blueprint)
    {
        return nullptr;
...
UK2Node_VariableSet* FUnrealAICommonUtils::CreateVariableSetNode(UEdGraph* Graph, UBlueprint* Blueprint, const FString& VariableName, const FVector2D& Position)
{
    if (!Graph || !Blueprint)
    {
        return nullptr;
...
UK2Node_InputAction* FUnrealAICommonUtils::CreateInputActionNode(UEdGraph* Graph, const FString& ActionName, const FVector2D& Position)
{
    if (!Graph)
    {
        return nullptr;
...
UK2Node_Self* FUnrealAICommonUtils::CreateSelfReferenceNode(UEdGraph* Graph, const FVector2D& Position)
{
    if (!Graph)
    {
        return nullptr;
...
bool FUnrealAICommonUtils::ConnectGraphNodes(UEdGraph* Graph, UEdGraphNode* SourceNode, const FString& SourcePinName, 
...
UEdGraphPin* FUnrealAICommonUtils::FindPin(UEdGraphNode* Node, const FString& PinName, EEdGraphPinDirection Direction)
{
    if (!Node)
    {
        return nullptr;
...
// Actor utilities
TSharedPtr<FJsonValue> FUnrealAICommonUtils::ActorToJson(AActor* Actor)
{
    if (!Actor)
    {
...
TSharedPtr<FJsonObject> FUnrealAICommonUtils::ActorToJsonObject(AActor* Actor, bool bDetailed)
{
    if (!Actor)
    {
        return nullptr;
...
UK2Node_Event* FUnrealAICommonUtils::FindExistingEventNode(UEdGraph* Graph, const FString& EventName)
{
    if (!Graph)
    {
        return nullptr;
...
bool FUnrealAICommonUtils::SetObjectProperty(UObject* Object, const FString& PropertyName, 
