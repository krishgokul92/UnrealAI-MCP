...
FEdGraphPinType FBPVariables::GetPinTypeFromString(const FString& TypeString)
{
    ...
    else if (TypeString.StartsWith(TEXT("/Script/")))
    {
        if (UEnum* Enum = FindObject<UEnum>(nullptr, *TypeString))
        {
            PinType.PinCategory = UEdGraphSchema_K2::PC_Byte;
            PinType.PinSubCategoryObject = Enum;
        }
        else if (UEnum* LoadedEnum = LoadObject<UEnum>(nullptr, *TypeString))
        {
            PinType.PinCategory = UEdGraphSchema_K2::PC_Byte;
