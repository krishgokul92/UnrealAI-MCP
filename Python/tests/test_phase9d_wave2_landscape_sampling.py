"""Live-editor smoke tests for the first Phase 9d landscape sampling slice."""
...
CREATE_LOCATION = [14000.0, 0.0, 0.0]
...
def _create_landscape() -> str:
    ...
    landscape_name = f"Landscape_Phase9D_Wave2_{suffix}"
...
class TestPhase9dWave2LandscapeSampling:
    def test_01_sample_landscape_point_after_flatten(self):
        ...
        payload = _ok(send_command("sample_landscape_point", {
            "landscape_name": landscape_name,
            "location": CREATE_LOCATION,
        }), f"sample_landscape_point({landscape_name})")

        assert payload.get("label") == landscape_name, payload
        assert payload.get("component_name"), payload
        assert isinstance(payload.get("component_extent"), list), payload
        assert isinstance(payload.get("layer_weights"), list), payload
        assert payload.get("layer_weight_count") == 0, payload
        assert payload.get("target_layer_count") == 0, payload
        assert abs(payload.get("height_world", -99999.0) - target_height) <= 4.0, payload