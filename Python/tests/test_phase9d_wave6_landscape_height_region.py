"""Live-editor smoke tests for the Phase 9d landscape height-region slice."""
...
CREATE_LOCATION = [46000.0, 0.0, 0.0]
...
def _create_landscape() -> str:
    ...
    landscape_name = f"Landscape_Phase9D_Wave6_{suffix}"
...
class TestPhase9dWave6LandscapeHeightRegion:
    def test_01_sample_landscape_height_region_after_flatten(self):
        ...
        payload = _ok(send_command("sample_landscape_height_region", {
            "landscape_name": landscape_name,
        ...
        assert abs(payload.get("height_min_world", -99999.0) - target_height) <= 4.0, payload
        assert abs(payload.get("height_max_world", -99999.0) - target_height) <= 4.0, payload

        height_rows = payload.get("height_rows") or []
        assert len(height_rows) == EXPECTED_COUNT_Y, payload
        for row in height_rows:
            assert len(row) == EXPECTED_COUNT_X, row
            for height_value in row:
                assert abs(height_value - target_height) <= 4.0, row